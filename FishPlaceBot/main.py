...
hi = [
    'Привет',
    'Где отдохнуть?',
    ' Подскажи, куда прокатиться на выходные?',
    'Куда поехать',
    'Куда поехать?',
...
def start(message):
    ...
    bot.send_message(
        message.chat.id,
        f'Привет! \u270B Хочешь отправиться на рыбалку?',
        parse_mode='Markdown',
        reply_markup=markup,
    )
...
def help(message):
    ...
    bot.send_message(
        message.chat.id,
        'Привет! Я подскажу где порыбачить',
        parse_mode='Markdown',
        reply_markup=markup,
    )
...
def get_user_text(message):
    ...
    if message.text == 'По регионам':
        ...
        # отправка сообщения
        bot.send_message(
            message.chat.id, 'Выберите регион', reply_markup=markup
        )
        return
    ...
    # если пользователь выбрал район
        ...
        # отправка сообщения
        bot.send_message(
            message.chat.id, 'Выберите место для рыбалки', reply_markup=markup
        )
        return
...
def yes_or_not(message):
    ...
    if message.text in yes:
        msg = bot.send_message(
            message.chat.id,
            f'Вот список регионов с классными местами для рыбалки. Выбирай.',
            parse_mode='Markdown',
        )
        # создание кнопок
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
    ...
    # если пользователь бот не понял пользователя
        ...
        msg = bot.send_message(
            message.chat.id, f'Я вас не понимаю.', parse_mode='Markdown'
        )
